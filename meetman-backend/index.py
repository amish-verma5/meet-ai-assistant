...
from qdrant_client.models import Filter, FieldCondition, Match, MatchValue
...
client = QdrantClient(path="vectors.qdrant")

if not client.collection_exists("document_vectors"):
...
vector_store = QdrantVectorStore(
    client=client, collection_name="document_vectors", embedding=embeddings
)
...
class AIClientAdapter:
    ...
    def chat_completions_create(
        self, model, messages, temperature=0.2, response_format=None
    ):
        # expect llama3.2 as the model name
        local = {"llama3.2": "llama3.2", "gpt-4o": "llama3.2"}
        groq = {"llama-3.3": "llama-3.3-70b-versatile", "llama-3.2": "llama3-70b-8192"}
        gemini = {
    ...
    def __init__(self, client_mode):
        ...
        if self.client_mode == "LOCAL":
            from fastembed import (
                TextEmbedding,
            )  # Import fastembed only when running project locally
    ...
    if word_count <= 50000:
        ...
        for i, chunk in enumerate(chunks):
            messages = [
                ...
                {"role": "user", "content": "Here is the transcript chunk: " + chunk},
            ]
...
def generate_everything(transcript):
    ...
    result = {"action_items": action_items, "notes": notes_content}
        ...
        messages = [
            {
                ...
                "content": f"""You are an executive assistant tasked with taking notes from an online meeting transcript. You must produce the notes in Markdown format
                    Full transcript: {transcript}. Follow the JSON structure:"""
                + "{notes: meeting notes}"
    ...
    if list_emails:
        ...
        for email in list_emails:
            ...
            if response.status_code != 200:
                return {
                    "type": "error",
                    "error": f"Error sending email to {email}: {response.text}",
                    "emails": None,
                }
            else:
...
def send_email(email, email_type, **kwargs):
    ...
    if email_type == "signup":
        ...
        subject = "Welcome to MeetMan - We're Glad You're Here!"
    ...
    payload = {"from": resend_email, "to": email, "subject": subject, "html": html}
    logger.info(f"Sending email to {email}")
...
async def upload_meeting_file(request):
    ...
    logger.info(
        f"Processing file upload for meeting_id: {meeting_id}, user_id: {user_id}"
    )
    ...
    if len(file_contents) > file_limit:
        ...
        return Response(
            status_code=413, description="File size exceeds 20MB limit", headers={}
        )
    ...
    storage_response = supabase.storage.from_("meeting_context_files").upload(
        unique_filename, file_contents
    )

    # Get public URL for the uploaded file
    file_url = supabase.storage.from_("meeting_context_files").get_public_url(
        unique_filename
    )
    ...
    new_entry = (
        ...
        .upsert(
            {"meeting_id": meeting_id, "user_id": user_id, "context_files": [file_url]},
...
def check_memory_enabled(user_id):
    try:
        result = (
            supabase.table("users").select("memory_enabled").eq("id", user_id).execute()
        )
...
async def end_meeting(request: Request, body: EndMeetingRequest):
    ...
    if not user_id:
        ...
        notes_content = res["notes"]
        action_items = res["action_items"]
        return {"notes_content": notes_content, "action_items": action_items}
    ...
    if memory and memory[0]["content"] and "ACTION_ITEMS" in memory[0]["content"]:
        ...
        return {"action_items": action_items, "notes_content": summary}
    else:
        ...
        pool.apply_async(
            store_memory_data, args=(memory_obj, user_id, meeting_obj_id, pool)
        )
...
async def generate_actions(request, body: ActionRequest):
    ...
    result = {"action_items": action_items, "notes_content": notes_content}
...
async def submit(request: Request, body: ActionItemsRequest):
    ...
    if successful_emails["type"] == "error":
        return {"successful_emails": None, "error": successful_emails["error"]}
...
async def track(request: Request, body: TrackingRequest):
    try:
        ...
        result = (
            ...
            .insert({"uuid": uuid, "event_type": event_type, "meeting_id": meeting_id})
            .execute()
        )
        return {"result": "success"}
    except Exception as e:
        return Response(
            status_code=500, description=f"Error tracking event: {str(e)}", headers={}
        )
...
def find_closest_chunk(query_embedding, chunks_embeddings, chunks):
    ...
    closest_indices = np.argsort(similarities, axis=1)[0, -5:][
        ::-1
    ]  # Five the closest indices of embeddings
...
def generate_realtime_suggestion(context, transcript):
    ...
    logger.info("GENERATING REALTIME SUGGESTION")
    response = ai_client.chat_completions_create(
        # model="llama-3.2",
        model="gpt-4o",
        messages=messages,
...
def check_suggestion(request_dict):
    try:
        ...
        if is_file_uploaded:
            sb_response = (
                supabase.table("meetings")
                .select("context_files, embeddings, chunks, suggestion_count")
                .eq("meeting_id", meeting_id)
                .eq("user_id", user_id)
                .execute()
                .data
            ...
            if "needs_help" in response_content and response_content["needs_help"]:
                ...
                closest_chunks = find_closest_chunk(
                    query_embedding=embedded_query,
                    chunks_embeddings=embedded_chunks,
                    chunks=file_chunks,
                )

                suggestion = generate_realtime_suggestion(
                    context=closest_chunks, transcript=transcript
                )
        ...
        return {
            "error": f"An unexpected error occurred. Please try again later. bitch: {e}"
        }
...
async def sync_meeting_with_supabase(meeting_id: str, user_id: str) -> str:
    ...
    try:
        ...
        logger.error(
            f"Error in Supabase operation for meeting {meeting_id}: {str(e)}",
            exc_info=True,
        )
        raise
...
async def on_connect(ws, msg):
    ...
    logger.info(
        f"WebSocket connection request - meeting_id: {meeting_id}, user_id: {user_id}"
    )
...
async def on_message(ws, msg):
    try:
        ...
        print("data is", data)
        logger.info(
            f"WebSocket message received - type: {type_}, meeting_id: {meeting_id}"
        )
...
async def store_transcript_file(transcript: str, meeting_obj_id: str):
    ...
    try:
        ...
        supabase.table("late_meeting").update({"transcript": file_url}).eq(
            "id", meeting_obj_id
        ).execute()
    except Exception as e:
...
def store_memory_data(
    ...
    try:
        ...
        emails_enabled = (
            ...
            .eq("id", user_id)
            .execute()
            .data[0]["emails_enabled"]
        )

        email_already_sent = (
            supabase.table("late_meeting")
            .select("post_email_sent")
            .eq("id", meeting_obj_id)
            .execute()
            .data[0]["post_email_sent"]
        )
...
def create_vectors(file_contents, unique_filename, user_id):
    logger.info(f"Starting create vectors for {unique_filename}")
    try:
        ...
        logger.info(f"File split into {len(texts)} chunks for {unique_filename}")
        if not texts:
            logger.warning(
                f"No text chunks found after splitting for {unique_filename}. Skipping embedding."
            )
            return
        ...
        documents = [
            Document(
                page_content=text,
                metadata={"source": unique_filename, "user_id": user_id},
            )
            for text in texts
        ]
        ...
        logger.error(f"Couldn't decode file contents for {unique_filename}: {e}")

    except Exception as e:
        logger.error(f"Error processing file {unique_filename}: {e}")
...
async def upload_file(request):
    ...
    Accepts a file, uploads it to Supabase Storage, and returns the file URL and metadata.
    """
    files = request.files
    file_name = list(files.keys())[0] if len(files) > 0 else None
    user_id = request.path_params.get("user_id")
    if not user_id:
        ...
        return Response(status_code=400, description="No user id provided", headers={})
    ...
    # Optionally, store metadata in a table (uncomment if needed)
    supabase.table("uploaded_files").insert(
        {"file_name": file_name, "file_url": file_url, "user_id": user_id}
    ).execute()

    threading.Thread(
        target=create_vectors,
        args=(file_contents, unique_filename, user_id),
        daemon=True,
    ).start()
...
async def generate_answer(request: Request):
    """
    Generation endpoint.
    Accepts a user query, retrieves relevant context from Qdrant, and generates an answer using Gemini.
    """
    try:
        ...
        if not user_id:
            logger.warning("No user_id provided in request")
            return Response(
                status_code=400, description="No user id provided", headers={}
            )
        ...
        # (Assumes 'embeddings' and 'vector_store' are already initialized as in your context)
        ...
        prompt = prompt_template.invoke({"question": user_query, "context": context})
        ...
        return Response(
            status_code=500,
            description=f"Internal server error: {e}",
            headers={},
        )
